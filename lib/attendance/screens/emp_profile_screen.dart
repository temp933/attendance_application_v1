...
//                   slivers: [
//                     SliverPadding(
//                       padding: EdgeInsets.fromLTRB(r.hPad, 12, r.hPad, 32),
//                       sliver: SliverToBoxAdapter(
//                         child: Center(
//                           child: ConstrainedBox(
//                             constraints: BoxConstraints(
...
//     _Row('Phone', _fmt(employeeData!['phone_number'])),
//     _Row('Emergency Contact', _fmt(employeeData!['emergency_contact'])),
//     _Row('Emg. Relation', _fmt(employeeData!['emergency_contact_relation'])),
//   ];

//   List<_Row> _workRows() => [
//     _Row('Designation', _fmt(employeeData!['designation_name'])),
...
//           : '-',
//     ),
//     _Row('Reporting To', _fmt(employeeData!['reporting_to_name'])),
//     _Row('Date of Joining', _fmtDate(employeeData!['date_of_joining'])),
//     _Row('Date of Relieving', _fmtDate(employeeData!['date_of_relieving'])),
//   ];
...
//     // Build sub-line: only unique non-dash values, no duplicates
//     final designation = _fmt(employeeData!['designation_name']);
//     final department = _fmt(employeeData!['department_name']);
//     final subParts = <String>{};
//     if (designation != '-') subParts.add(designation);
//     if (department != '-') subParts.add(department);
...
//                     _heroStat(exp != null ? '$exp yrs' : '-', 'Experience'),
//                     _heroVDiv(),
//                     _heroStat(joined, 'Joined'),
//                     _heroVDiv(),
//                     _heroStat(empType != '-' ? empType : '-', 'Type'),
//                   ],
//                 ),
//               ),
//             ],
...
//   Widget _buildAvatar(Responsive r) {
//     return FutureBuilder<http.Response>(
//       future: _photoFuture ?? Future.value(http.Response('', 404)),
//       builder: (context, snap) {
//         final hasPhoto =
//             snap.hasData &&
//             snap.data!.statusCode == 200 &&
...
//           height: r.avatarSize,
//           decoration: BoxDecoration(
//             borderRadius: BorderRadius.circular(r.isDesktop ? 22 : 18),
//             color: Colors.white.withOpacity(0.15),
//             border: Border.all(color: Colors.white.withOpacity(0.35), width: 2),
//           ),
//           child: ClipRRect(
//             borderRadius: BorderRadius.circular(r.isDesktop ? 20 : 16),
//             child: hasPhoto
//                 ? Image.memory(snap.data!.bodyBytes, fit: BoxFit.cover)
//                 : Center(
//                     child: Text(
//                       _initials(),
//                       style: TextStyle(
...
//   // ─── Info Row: label LEFT (fixed 110px), value RIGHT ─────────────────────────
//   Widget _buildInfoRow(Responsive r, _Row row) {
//     final isEmpty = row.value == '-';
...
//         decoration: BoxDecoration(
//           color: row.pillColor.withOpacity(0.10),
...
//           // Value fills remaining width, right-aligned
//           Expanded(
//             child: Align(
//               alignment: row.pill
...
//     final level = edu['education_level']?.toString() ?? '';
//     final stream = edu['stream']?.toString().trim() ?? '';
//     final score = edu['score'] != null ? '${edu['score']}%' : null;
//     final year = edu['year_of_passout']?.toString().trim() ?? '';
//     final college = edu['college_name']?.toString().trim() ?? '';
//     final uni = edu['university']?.toString().trim() ?? '';
//     final color = _eduColor(level);
  ...
  Widget _buildInfoRow(Responsive r, _Row row) {
    ...
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // Label takes 42% of available width
          Expanded(
            flex: 42,
            child: Text(
              row.label,
          ...
          const SizedBox(width: 8),
          // Value takes 58% of available width
          Expanded(
            flex: 58,
            child: Align(
              alignment: row.pill
