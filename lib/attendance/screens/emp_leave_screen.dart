...
class _LeaveScreenState extends State<LeaveScreen>
  ...
  Widget _buildBalanceBar() {
    final paid = _myBalances
        .where((b) => (b['allocated_days'] as num? ?? 0) > 0)
        .toList();
      ...
      child: Column(
        ...
        children: [
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: paid.map((b) {
                final avail = (b['available_days'] as num? ?? 0).toDouble();
                final alloc = (b['allocated_days'] as num? ?? 0).toDouble();
                final used = (b['used_days'] as num? ?? 0).toDouble();
                final pending = (b['pending_days'] as num? ?? 0).toDouble();
                final pct = alloc > 0 ? (avail / alloc).clamp(0.0, 1.0) : 0.0;
                final color = pct > 0.5
                    ? _accent
                    : pct > 0.2
                    ? _amber
                    : _red;
                final availStr = avail % 1 == 0
                    ? avail.toInt().toString()
                    : avail.toStringAsFixed(1);
                final allocStr = alloc % 1 == 0
                    ? alloc.toInt().toString()
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (pending > 0) ...[
                        ...
                        Text(
                          '${pending % 1 == 0 ? pending.toInt() : pending.toStringAsFixed(1)} pending',
                          style: const TextStyle(
                            fontSize: 10,
                            color: _amber,
...
class _ApplyLeaveSheetState extends State<_ApplyLeaveSheet> {
  ...
  Map<int, Map<String, dynamic>> _balanceMap = {};
  bool _loadingBalances = true;
  String? _formError; // inline error message
  ...
  Future<void> _fetchBalances() async {
    try {
      final res = await ApiClient.get('/leave/balance/my');
      if (!mounted) return;
      final body = jsonDecode(res.body) as Map<String, dynamic>;
      if (body['ok'] == true) {
        ...
        for (final b in list) {
          final id = b['leave_type_id'];
          if (id != null) map[id as int] = b;
        }
        if (mounted) setState(() => _balanceMap = map);
      }
    } catch (_) {
    } finally {
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Builder(
                                      builder: (_) {
                                        final typeId =
                                            t['leave_type_id'] as int?;
                                        final bal = typeId != null
                                            ? _balanceMap[typeId]
                                            : null;
                                        if (bal == null)
                                          return const SizedBox.shrink();
                                        final avail =
                                            (bal['available_days'] as num? ?? 0)
                                                .toDouble();
                                        final alloc =
                                            (bal['allocated_days'] as num? ?? 0)
                                                .toDouble();
                                        if (alloc == 0)
                                          return const SizedBox.shrink();
                                        final availStr = avail % 1 == 0
              ...
              Align(
                ...
                child: Builder(
                  builder: (_) {
                    ...
                    final isZero = !_isHalfDay && _days == 0;
                    return Container(
                      padding: const EdgeInsets.symmetric(
                        horizontal: 12,
                        vertical: 5,
                      ...
                      decoration: BoxDecoration(
                        color: isZero ? _red.withOpacity(0.08) : _primaryLight,
                        borderRadius: BorderRadius.circular(20),
                      ),
                      child: Text(
                        label,
