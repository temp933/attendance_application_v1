...
class AdminFaceApprovalPage extends StatefulWidget {
  ...
  @override
  State<AdminFaceApprovalPage> createState() => _AdminFaceApprovalPageState();
}

class _AdminFaceApprovalPageState extends State<AdminFaceApprovalPage> {
  List _requests = [];
  bool _loading = true;
  String? _error;
  ...
  Future<void> _fetchRequests() async {
    ...
    try {
      final res = await ApiClient.get('/admin/face/pending-requests');
  ...
  Future<http.Response> _resolvePhoto(
    ...
    if (requestId != null) {
      final res = await ApiClient.get(
        '/admin/face/pending-request/$requestId/photo',
      );
      ...
      child: InkWell(
        ...
        onTap: () async {
          final result = await Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => FaceApprovalDetailPage(request: widget.request),
            ),
          );
...
// ─────────────────────────────────────────────────────────────────────────────
class FaceApprovalDetailPage extends StatefulWidget {
  final Map request;
  const FaceApprovalDetailPage({super.key, required this.request});
  @override
  State<FaceApprovalDetailPage> createState() => _FaceApprovalDetailPageState();
}
...
class _FaceApprovalDetailPageState extends State<FaceApprovalDetailPage> {
  ...
  Future<void> _approve(BuildContext context) async {
    ...
    final res = await ApiClient.post('/admin/face/approve-request', {
        ...
        actions: [
          ...
          FilledButton(
            ...
            onPressed: () async {
              ...
              await ApiClient.post('/admin/face/reject-request', {
                'request_id': widget.request['request_id'],
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _infoTile(
                        ...
                        value:
                            widget.request['reporting_to_employee_id']
                                ?.toString() ??
                            '-',
                        highlight: _changed('reporting_to_employee_id'),
                        oldValue:
                            _current?['reporting_to_employee_id']?.toString() ??
                            '',
                      ),
                      _dividerRow(),
                      _infoTile(
