...
// ─────────────────────────────────────────────────────────────────────────────
// Initialize background service (call once from main.dart or app init)
// ─────────────────────────────────────────────────────────────────────────────
...
class _GpsAttendanceScreenState extends State<GpsAttendanceScreen>
  ...
  Future<void> _fetchToday() async {
    ...
    try {
      final res = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/gps/today'),
  ...
  Future<void> _fetchHistory() async {
    ...
    try {
      final res = await http.get(
        Uri.parse('${ApiConfig.baseUrl}/gps/history?limit=7'),
        headers: ApiConfig.headers,
      );
      final body = jsonDecode(res.body) as Map<String, dynamic>;
  ...
  Future<void> _checkIn() async {
    ...
    try {
      ...
      final res = await http.post(
        Uri.parse('${ApiConfig.baseUrl}/gps/checkin'),
        headers: ApiConfig.headers,
        body: jsonEncode({
          'latitude': position.latitude,
          'longitude': position.longitude,
  ...
  Future<void> _checkOut() async {
    ...
    try {
      ...
      final res = await http.post(
        Uri.parse('${ApiConfig.baseUrl}/gps/checkout'),
        headers: ApiConfig.headers,
        body: jsonEncode({
          'latitude': position.latitude,
          'longitude': position.longitude,
  ...
  Widget _buildStatusCard() {
    ...
    if (isActive) {
      ...
      sublabel = 'Tap CHECK IN to start GPS tracking';
    }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          // Location update indicator when active
          if (isActive) ...[
