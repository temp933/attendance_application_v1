...
class FaceGpsAttendanceRecord {
  ...
  factory FaceGpsAttendanceRecord.fromJson(Map<String, dynamic> j) {
    ...
    return FaceGpsAttendanceRecord(
...
class FaceGpsAttendanceManagementService {
  static Future<Map<String, dynamic>> fetchAll({
    ...
    final uri = Uri.parse('${ApiConfig.baseUrl}/face/admin/all').replace(
      queryParameters: {
        'date': date,
        'status': status,
        'search': search,
...
class FaceGpsAttendanceManagementScreen extends StatefulWidget {
  ...
  @override
  State<FaceGpsAttendanceManagementScreen> createState() =>
      _FaceGpsAttendanceManagementScreenState();
}

class _FaceGpsAttendanceManagementScreenState
    extends State<FaceGpsAttendanceManagementScreen> {
  bool _loading = true;
  String? _error;
  List<FaceGpsAttendanceRecord> _records = [];
  Map<int, List<FaceGpsAttendanceRecord>> _groupedRecords = {};
  List<int> _employeeOrder = [];
  FaceGpsAttendanceSummaryStats? _stats;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final data = await FaceGpsAttendanceManagementService.fetchAll(
      ...
      final rawRecords = (data['records'] as List? ?? [])
          .map(
            (e) => FaceGpsAttendanceRecord.fromJson(e as Map<String, dynamic>),
          )
          .toList();

      // Group by employee
      final grouped = <int, List<FaceGpsAttendanceRecord>>{};
      final order = <int>[];
      ...
      setState(() {
        ...
        _stats = FaceGpsAttendanceSummaryStats(
...
class _GpsEmployeeCard extends StatefulWidget {
  final List<FaceGpsAttendanceRecord> sessions;
...
class _GpsEmployeeCardState extends State<_GpsEmployeeCard>
  ...
  FaceGpsAttendanceRecord get _latest => widget.sessions.first;
  FaceGpsAttendanceRecord get _oldest => widget.sessions.last;
...
class _LocationRow extends StatelessWidget {
  final FaceGpsAttendanceRecord session;
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Icon(Icons.location_pin, size: 13, color: _primary),
                  const SizedBox(width: 4),
                  const Text(
                    'Pin',
                    style: TextStyle(
