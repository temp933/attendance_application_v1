...
// ─────────────────────────────────────────────────────────────────────────────
// Status code helpers (mirrors report_screen.dart)
// ─────────────────────────────────────────────────────────────────────────────

// Matrix cell colors — grouped by family for quick visual scanning.
//   GREEN   = present, fully paid     → P, PL, PH, PLH
//   YELLOW  = present + unpaid half   → PU, PLU
...
const Map<String, (String, String)> _statusColors = {
  'P': ('FFECFDF5', 'FF16A34A'),
  'PL': ('FFECFDF5', 'FFB45309'),
  'PH': ('FFCCFBF1', 'FF0D9488'),
  'PLH': ('FFCCFBF1', 'FFB45309'),
  'PU': ('FFFEF9C3', 'FF65A30D'),
  'PLU': ('FFFEF9C3', 'FFB45309'),
  'HD': ('FFE0F2FE', 'FF0369A1'),
  'HU': ('FFFCE7F3', 'FFBE185D'),
  'L': ('FFFFE4E6', 'FFBE123C'),
  'A': ('FFFEF2F2', 'FFDC2626'),
  'H': ('FFDBEAFE', 'FF1D4ED8'),
  'W': ('FFF5F3FF', 'FF7C3AED'),
...
// Short text shown inside each matrix day cell
...
// Medium label for the Day-Wise / Site-Entry status column
...
String _statusLabel(EmpDaily emp) =>
    _statusShortLabel[emp.status] ?? emp.statusLabel;

// Format a decimal day-count: trims ".0" but keeps ".5"
String _fmtNum(double v) =>
    v == v.roundToDouble() ? v.toInt().toString() : v.toStringAsFixed(1);

// Format late minutes as "Xh Ym" / "--"
String _fmtLateHrs(int lateMinutes) {
...
class ReportService {
  ...
  static Future<void> _exportDailyNormal(
    ...
    sheet.merge(
      ...
      xl.CellIndex.indexByColumnRow(columnIndex: lastCol, rowIndex: 1),
    );
    final legendCell = sheet.cell(
      xl.CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 1),
    );
    legendCell.value = xl.TextCellValue(
      'P=Present  PL=Late  P½=Half Leave  HD=Half-Day(Paid)  HU=Half-Day(Unpaid)  '
      'L=Leave  A=Absent  H=Holiday  W=Weekend  C=Comp-Off',
    );
    legendCell.cellStyle = _subtitleStyle();
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      final rowIdx = i + 3;
      final rowBg = i.isEven ? 'FFFFFFFF' : 'FFF8FAFF';
      final sc = _statusColors[emp.status] ?? (rowBg, 'FF000000');
      final statusText = _statusLabel(emp);
      ...
      final cellDefs = [
        ...
        (statusText, sc.$1, sc.$2, true, false),
        (
          emp.isLate ? 'Yes' : 'No',
          emp.isLate ? 'FFFEF3C7' : rowBg,
          emp.isLate ? 'FFB45309' : 'FF94A3B8',
    ...
    final widths = [
      ...
      12.0,
      14.0,
      22.0, // Status — wider for longer labels like "Present + ½ Unpaid (Late)"
      8.0,
      16.0,
      20.0,
    ];
  ...
  static Future<void> _exportDailySiteEntry(
    ...
    const widths = [
      ...
      24.0, // Name
      18.0, // Department
      22.0, // Status — wider for longer labels
      9.0, // Session #
      22.0, // Site Name
      14.0, // Check In
      14.0, // Check Out
...
// ─────────────────────────────────────────────────────────────────────────────
//
// Columns: S.No | EmpID | Name | [dates] | Present | Absent | Leave |
//          CO Earned | CO Used | CO Expired |
//          Paid Leave | Unpaid Leave | Half Day | Late Days | Late Hrs | Att%
//
// index:     0      1      2     3..3+n-1   3+n      4+n     5+n
//            6+n      7+n      8+n
...
class MatrixExcelBuilder {
  static xl.Excel build(
    ...
    final n = dates.length;
    final lastCol = 14 + n; // 0-based — 12 summary cols after name+dates
    ...
    _setCell(
      ...
      1,
      0,
      'P=Present  PL=Late  P½=Half Leave (Paid/Unpaid)  '
      'HD=Half-Day Only(Paid)  HU=Half-Day Only(Unpaid)  '
      'L=Full Leave  A=Absent  H=Holiday  W=Weekend  C=Comp-Off',
      _subtitleStyle(),
    );
    s.setRowHeight(1, 16);
    ...
    for (int c = 0; c < 12; c++) _setCell(s, 2, 3 + n + c, '', _hdrStyle());
    s.setRowHeight(2, 16);
    ...
    _setCell(s, 3, 4 + n, 'Absent', _hdrStyle());
    _setCell(s, 3, 5 + n, 'Leave', _hdrStyle());
    _setCell(s, 3, 6 + n, 'CO Earned', _hdrStyle(hex: 'FFD97706'));
    _setCell(s, 3, 7 + n, 'CO Used', _hdrStyle(hex: 'FFB45309'));
    _setCell(s, 3, 8 + n, 'CO Expired', _hdrStyle(hex: 'FFDC2626'));
    _setCell(s, 3, 9 + n, 'Paid Leave', _hdrStyle(hex: 'FF0369A1'));
    _setCell(s, 3, 10 + n, 'Unpaid Leave', _hdrStyle(hex: 'FF7C3AED'));
    _setCell(s, 3, 11 + n, 'Half Day', _hdrStyle(hex: 'FFBE185D'));
    _setCell(s, 3, 12 + n, 'Late Days', _hdrStyle(hex: 'FFB45309'));
    _setCell(s, 3, 13 + n, 'Late Hrs', _hdrStyle(hex: 'FFB45309'));
    _setCell(s, 3, 14 + n, 'Att %', _hdrStyle());
    s.setRowHeight(3, 28);
    ...
    for (int c = 0; c < summaryWidths.length; c++) {
      s.setColumnWidth(3 + n + c, summaryWidths[c]);
    }
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      // Day cells — use grouped status colors + short cell text
      for (int j = 0; j < emp.days.length; j++) {
        final code = emp.days[j];
        final c = _statusColors[code] ?? (rowBg, 'FF000000');
        final bold = code == 'P' || code == 'PH' || code == 'HD' || code == 'C';
        _setCell(
          s,
          row,
          3 + j,
          _cellText(code),
          _cellStyle(center: true, bg: c.$1, fg: c.$2, bold: bold),
        );
      }
      ...
      _setCell(
        ...
        _cellStyle(
          center: true,
          bg: emp.compOffEarned > 0 ? 'FFFFFBEB' : rowBg,
          fg: emp.compOffEarned > 0 ? 'FFD97706' : 'FF000000',
        ),
      );
      // CO Used
      ...
      _setCell(
        ...
        _cellStyle(
          center: true,
          bg: emp.paidLeaveDays > 0 ? 'FFE0F2FE' : rowBg,
          fg: emp.paidLeaveDays > 0 ? 'FF0369A1' : 'FF000000',
        ),
      );
      ...
      _setCell(
        ...
        _cellStyle(
          center: true,
          bg: emp.unpaidLeaveDays > 0 ? 'FFF5F3FF' : rowBg,
          fg: emp.unpaidLeaveDays > 0 ? 'FF7C3AED' : 'FF000000',
        ),
      );
      // Half Day (count, paid + unpaid)
      _setCell(
        s,
        row,
        11 + n,
        ...
        _cellStyle(
          center: true,
          bg: emp.halfDayCount > 0 ? 'FFFCE7F3' : rowBg,
          fg: emp.halfDayCount > 0 ? 'FFBE185D' : 'FF000000',
        ),
      );
      // Late Days
      _setCell(
        ...
        _cellStyle(
          ...
          fg: emp.lateDays > 0 ? 'FFB45309' : 'FF000000',
        ),
      );
      // Late Hrs
      _setCell(
...
// ─────────────────────────────────────────────────────────────────────────────
// Daily Excel Builder (legacy / unused by exportDaily — kept for reference)
// ─────────────────────────────────────────────────────────────────────────────
...
class DailyExcelBuilder {
  static xl.Excel build(List<EmpDaily> data, DateTime date) {
    ...
    const widths = [
      ...
      13.0, // Check Out
      13.0, // Worked Hrs (wider for 8h 30m 15s)
      20.0, // Status — wider for new labels
      8.0,
      12.0, // Late By
      12.0,
      18.0,
    ...
    if (data.isNotEmpty) {
      final tr = data.length + 3;
      final present = data.where((e) => e.isPresentStatus).length;
      final absent = data.where((e) => e.status == 'A').length;
      final onLeave = data
          .where((e) => e.status == 'L' || e.status == 'HD' || e.status == 'HU')
          .length;
      final compOff = data.where((e) => e.compOffEarned).length;
      final late = data.where((e) => e.isLate).length;
