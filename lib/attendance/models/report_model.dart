...
//     paidLeaveDays: parseInt(j['paid_leave_days']),
//     unpaidLeaveDays: parseInt(j['unpaid_leave_days']),
//     halfDayCount: parseInt(j['half_day_count']),
//   );
// }
...
class MatrixEmp {
  ...
  // ── core summary (decimal — supports half-day = 0.5) ────────────────────
  ...
  // ── leave breakdown (salary-relevant) ────────────────────────────────────
  final double paidLeaveDays; // includes half-day paid * 0.5
  final double unpaidLeaveDays; // includes half-day unpaid * 0.5
  final int paidHalfDays; // count of paid half-day leaves
  final int unpaidHalfDays; // count of unpaid half-day leaves
  final int halfDayCount; // paidHalfDays + unpaidHalfDays

  // ── salary reference ─────────────────────────────────────────────────────
  final double deductionDays; // absent + unpaid full leave + unpaid half*0.5
  ...
  factory MatrixEmp.fromJson(Map<String, dynamic> j) => MatrixEmp(
    ...
    department: parseString(j['department']),
    days: List<String>.from(j['days'] ?? []),
    presentDays: parseDouble(j['present_days']),
    absentDays: parseInt(j['absent_days']),
    leaveDays: parseDouble(j['leave_days']),
    compOffDays: parseInt(j['comp_off_days']),
    totalWorkingDays: parseInt(j['total_working_days']),
    percentage: parseDouble(j['percentage']),
    compOffEarned: parseInt(j['comp_off_earned']),
    ...
    lateDays: parseInt(j['late_days']),
    lateMinutes: parseInt(j['late_minutes']),
    paidLeaveDays: parseDouble(j['paid_leave_days']),
    unpaidLeaveDays: parseDouble(j['unpaid_leave_days']),
    paidHalfDays: parseInt(j['paid_half_days']),
    unpaidHalfDays: parseInt(j['unpaid_half_days']),
    halfDayCount: parseInt(j['half_day_count']),
    deductionDays: parseDouble(j['deduction_days']),
  );
}
...
class EmpDaily {
  ...
  /// Short status code from backend:
  /// P, PL, PH, PLH, PU, PLU, HD, HU, L, A, H, W, C
  final String status;

  /// Human-readable label, e.g. "Present + Half Leave (Late)"
  ...
  final int totalCompOffUsed;
  final int totalCompOffExpired;
  final List<SiteSession> siteSessions;
  ...
  factory EmpDaily.fromJson(Map<String, dynamic> j) => EmpDaily(
    ...
    workedMinutes: j['worked_seconds'] != null
        ? (parseInt(j['worked_seconds']) ~/ 60)
        : parseInt(j['worked_minutes']),
    status: parseString(j['status']).isEmpty ? 'A' : parseString(j['status']),
    statusLabel: parseString(j['status_label']).isEmpty
        ? 'Absent'
        : parseString(j['status_label']),
    isLate: j['is_late'] == true,
    lateMinutes: parseInt(j['late_minutes']),
    compOffEarned: j['comp_off_earned'] == true,
    holidayName: j['holiday_name']?.toString(),
  ...
  /// True if this status code involves the employee being physically present.
  bool get isPresentStatus =>
      status == 'P' ||
      status == 'PL' ||
      status == 'PH' ||
      status == 'PLH' ||
...
class AttendanceStatus {
  static const present = 'P';
  static const presentLate = 'PL';
  static const presentPaidHalf = 'PH';
  static const presentLatePaidHalf = 'PLH';
  static const presentUnpaidHalf = 'PU';
  static const presentLateUnpaidHalf = 'PLU';
  static const halfDayPaid = 'HD';
  static const halfDayUnpaid = 'HU';
  static const leave = 'L';
  static const absent = 'A';
  static const holiday = 'H';
  static const weekend = 'W';
