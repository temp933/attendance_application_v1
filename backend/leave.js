...
router.post("/apply", async (req, res) => {
  ...
  try {
    ...
    const approvalLevels =
      !policy.requires_approval || isCompOff
        ? 0
        : (matchedRule?.approval_levels ?? 0);
    ...
    const hasNoManager = !empInfo?.reporting_to_employee_id;
    ...
    // If manager exists but chain is still empty — genuine error
    if (chain.length === 0) {
      await conn.rollback();
      return send(
        res,
        422,
        false,
        "Could not resolve any approver. Employee has no reporting manager configured.",
      );
    }
