  ...
  try {
    ...
    if (existingRows.length > 0) {
      return res.status(409).json({
        message:
          "Email address(es) or contact number are already registered with another organisation.",
      });
    }
...
app.post("/api/auth/complete", completeLimiter, async (req, res) => {
  ...
  if (dupCheck.length > 0) {
    return res.status(409).json({
      message: "Organisation, email, or contact number already registered.",
    });
  }
  ...
  try {
    ...
    if (existingOrg) {
      ...
      return res.status(409).json({
        message:
          "Organisation already exists (duplicate email or contact number).",
      });
    }
    ...
    // ── 7. Fetch company_code (auto-generated by DB trigger) ──────────────
    ...
    const MODE_MODULES = {
      1: [
        "emp_attendance_normal",
        "admin_attendance_normal",
        "approval",
        ...(enableSite ? ["site_management"] : []),
      ],
      2: [
        "emp_attendance_gps",
        "admin_attendance_gps",
        "approval",
        ...(enableSite ? ["site_management"] : []),
      ],
      3: [
        "emp_attendance_face",
        "admin_attendance_face",
        "face_approval",
        ...(enableSite ? ["site_management"] : []),
      ],
      4: [
...
// site entry routes
const siteEntryRoutes = require("./Attendance/site_entry_routes");
...
const { router: orgNotifRoutes, runOrgNotifCron } = require("./org_notification_routes");
app.use("/api/org-notifications", authMiddleware, orgNotifRoutes);

const cron = require("node-cron");
cron.schedule("* * * * *", runOrgNotifCron, { timezone: "Asia/Kolkata" });
console.log("[org-notify] Scheduled notification cron started.");
// ── Stubs
...
const userDashboard = require("./user_dashboard");
