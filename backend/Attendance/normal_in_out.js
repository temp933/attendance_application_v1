...
function normalizeRecord(r) {
  ...
  // ADD THIS — extract just the date portion in IST
  if (r.work_date instanceof Date) {
    r.work_date = toISTStr(r.work_date).slice(0, 10); // "2026-05-16"
  }
  return r;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// GET /api/attendance/policy          — fetch current policy
// ─────────────────────────────────────────────────────────────────────────────
router.get("/policy", requireAuth, async (req, res) => {
  const { tenantId } = req.user;
  try {
    const policy = await getPolicy(tenantId); // reuse your existing helper
    ...
    if (existing) {
      ...
      await db.query(
        ...
        [
          ...
          halfday_after_minutes,
          overtime_after_minutes,
          multiple_in_out_allowed ? 1 : 0,
          auto_checkout_enabled ? 1 : 0,
          tenantId,
        ],
      );
    } else {
      // INSERT first-time policy
      await db.query(
        `INSERT INTO attendance_policy
           (tenant_id, office_in_time, office_out_time,
