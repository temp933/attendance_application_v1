...
// async function fetchLeaveBreakdownMap(tenantId, empIds, from, to) {
//   if (!empIds.length) return new Map();
//   const [rows] = await db.query(
//     `SELECT
//        lm.emp_id,
//        SUM(lt.is_paid = 1 AND lm.final_status = 'Approved' AND lm.is_half_day = 0) AS paid_days,
//        SUM(lt.is_paid = 0 AND lm.final_status = 'Approved' AND lm.is_half_day = 0) AS unpaid_days,
//        SUM(lm.is_half_day = 1 AND lm.final_status = 'Approved')                    AS half_day_count,
//        SUM(lm.final_status = 'Approved' AND lm.is_half_day = 0)                    AS total_leave_days
//      FROM leave_master lm
//      JOIN leave_type_master lt
//        ON lm.leave_type_id = lt.leave_type_id AND lt.tenant_id = lm.tenant_id
//      WHERE lm.tenant_id = ? AND lm.emp_id IN (?)
//        AND lm.leave_start_date <= ? AND lm.leave_end_date >= ?
//        AND lm.final_status = 'Approved'
//      GROUP BY lm.emp_id`,
//     [tenantId, empIds, to, from],
//   );
//   const map = new Map();
//   for (const r of rows)
//     map.set(r.emp_id, {
...
//       halfDayCount: Number(r.half_day_count ?? 0),
//       totalLeaveDays: Number(r.total_leave_days ?? 0),
//     });
//   return map;
// }
...
//       ),
//       fetchLeaveBreakdownMap(tenant_id, empIds, from, to),
//     ]);
...
//       const lvBreak = leaveBreakdownMap.get(emp.emp_id) ?? {
//         paidDays: 0,
//         unpaidDays: 0,
//         halfDayCount: 0,
//         totalLeaveDays: 0,
...
//         paid_leave_days: lvBreak.paidDays,
//         unpaid_leave_days: lvBreak.unpaidDays,
//         half_day_count: lvBreak.halfDayCount,
//       };
//     });
...
async function fetchPolicy(tenantId) {
  ...
  return {
    isSatWeekoff: row?.is_saturday_weekoff == 1,
    isSunWeekoff: row?.is_sunday_weekoff == 1,
    compOffEnabled: row?.comp_off_enabled == 1,
  };
}
...
// ─────────────────────────────────────────────────────────────────────────────
// fetchLeaveMap — returns TWO maps:
//   fullLeaveMap  : emp_id → Set<date>   (full-day approved leaves only)
//   halfDayLeaveMap: emp_id → Map<date, { isPaid: bool }>  (half-day leaves)
// ─────────────────────────────────────────────────────────────────────────────
async function fetchLeaveMap(tenantId, from, to) {
  const [rows] = await db.query(
    `SELECT
       lm.emp_id,
       DATE_FORMAT(lm.leave_start_date, '%Y-%m-%d') AS start_date,
       DATE_FORMAT(lm.leave_end_date,   '%Y-%m-%d') AS end_date,
       lm.is_half_day,
       COALESCE(lt.is_paid, 1)                      AS is_paid
     FROM leave_master lm
     LEFT JOIN leave_type_master lt
       ON lt.leave_type_id = lm.leave_type_id AND lt.tenant_id = lm.tenant_id
     WHERE lm.tenant_id = ? AND lm.final_status = 'Approved'
       AND lm.leave_start_date <= ? AND lm.leave_end_date >= ?`,
    [tenantId, to, from],
  );

  const fullLeaveMap = new Map(); // emp_id → Set<date>
  const halfDayLeaveMap = new Map(); // emp_id → Map<date, { isPaid }>

  for (const row of rows) {
    const isPaid = row.is_paid == 1;

    if (row.is_half_day == 1) {
      // Half-day: always a single date (start == end)
      const date = row.start_date;
      if (!halfDayLeaveMap.has(row.emp_id))
        halfDayLeaveMap.set(row.emp_id, new Map());
      halfDayLeaveMap.get(row.emp_id).set(date, { isPaid });
    } else {
      // Full-day: expand date range
      if (!fullLeaveMap.has(row.emp_id))
        fullLeaveMap.set(row.emp_id, new Set());
      const set = fullLeaveMap.get(row.emp_id);
      const cur = new Date(row.start_date);
      const end = new Date(row.end_date);
      while (cur <= end) {
        set.add(cur.toISOString().slice(0, 10));
  ...
  return { fullLeaveMap, halfDayLeaveMap };
}
...
async function fetchAttendanceMap(tenantId, from, to, mode = "normal") {
  ...
  const map = new Map(); // emp_id → Set<date>
  const lateMap = new Map(); // emp_id → Set<date>
  for (const row of rows) {
    if (!map.has(row.employee_id)) map.set(row.employee_id, new Set());
    map.get(row.employee_id).add(row.work_date);
    if (row.is_late == 1) {
...
// ─────────────────────────────────────────────────────────────────────────────
// fetchLeaveBreakdownMap — salary-correct counts
//
// paid_full_days    : full-day paid leave days (each = 1.0)
// unpaid_full_days  : full-day unpaid leave days (each = 1.0, deduct 1 day)
// paid_half_days    : count of paid half-day leaves (each = 0.5 day value)
// unpaid_half_days  : count of unpaid half-day leaves (each = 0.5 day, deduct 0.5)
// half_day_count    : total half-day leaves (paid + unpaid) — for display
// ─────────────────────────────────────────────────────────────────────────────
async function fetchLeaveBreakdownMap(tenantId, empIds, from, to) {
  if (!empIds.length) return new Map();
  const [rows] = await db.query(
    `SELECT
       lm.emp_id,
       SUM(lm.is_half_day = 0 AND lt.is_paid = 1  AND lm.final_status = 'Approved') AS paid_full_days,
       SUM(lm.is_half_day = 0 AND lt.is_paid = 0  AND lm.final_status = 'Approved') AS unpaid_full_days,
       SUM(lm.is_half_day = 1 AND lt.is_paid = 1  AND lm.final_status = 'Approved') AS paid_half_days,
       SUM(lm.is_half_day = 1 AND lt.is_paid = 0  AND lm.final_status = 'Approved') AS unpaid_half_days
     FROM leave_master lm
     JOIN leave_type_master lt
       ON lm.leave_type_id = lt.leave_type_id AND lt.tenant_id = lm.tenant_id
     WHERE lm.tenant_id = ? AND lm.emp_id IN (?)
  ...
  const map = new Map();
  for (const r of rows) {
    const paidFull = Number(r.paid_full_days ?? 0);
    const unpaidFull = Number(r.unpaid_full_days ?? 0);
    const paidHalf = Number(r.paid_half_days ?? 0);
    const unpaidHalf = Number(r.unpaid_half_days ?? 0);
    map.set(r.emp_id, {
      paidFullDays: paidFull,
      unpaidFullDays: unpaidFull,
      paidHalfDays: paidHalf, // count (each = 0.5 day)
      unpaidHalfDays: unpaidHalf, // count (each = 0.5 day, salary deducted)
      halfDayCount: paidHalf + unpaidHalf,
      // Decimal values for summary display
      paidLeaveDays: paidFull + paidHalf * 0.5,
      unpaidLeaveDays: unpaidFull + unpaidHalf * 0.5,
    });
  }
  return map;
}
...
//
// Cell codes:
//   P    = present (full day, no leave)
//   PL   = present + late (no leave)
//   PH   = present + paid half-day leave same day
//   PLH  = present + late + paid half-day leave
//   PU   = present + unpaid half-day leave same day
//   PLU  = present + late + unpaid half-day leave
//   HD   = half-day paid leave only (absent other half)
//   HU   = half-day unpaid leave only (absent other half)
//   L    = full-day leave (paid or unpaid — summary cols distinguish)
//   A    = absent
//   H    = holiday
//   W    = weekend
//   C    = comp-off used
//
// Priority: present overrides everything except half-day blend
// ─────────────────────────────────────────────────────────────────────────────
function resolveStatus({
  isPresent,
  isLate,
  ...
  isWeekend,
  isFullLeave,
  halfDay, // null | { isPaid: bool }
}) {
  // Non-working days — ignore leave/attendance
  if (isHoliday) return "H";
  if (isWeekend) return "W";

  if (isPresent) {
    if (halfDay) {
      // Present + half-day leave on same day
      if (halfDay.isPaid)
        return isLate ? "PLH" : "PH"; // Present + Paid Half
      else return isLate ? "PLU" : "PU"; // Present + Unpaid Half
    }
    return isLate ? "PL" : "P";
  }
  ...
  if (isCompOff) return "C";
  if (halfDay) {
    return halfDay.isPaid ? "HD" : "HU"; // Half-day leave only (absent other half)
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Salary-correct effective present calculator
//
// effectivePresent is used for Att% only.
// Salary deduction days are computed separately.
//
// P/PL       = 1.0
// PH/PLH     = 1.0  (came + paid half leave = full day)
// PU/PLU     = 1.0  (came = present; unpaid half = salary deduct separately)
// HD         = 1.0  (paid half leave = full pay)
// HU         = 0.5  (unpaid half = only 0.5 present value)
// C          = 1.0
// L/A/H/W    = 0.0 (H/W don't affect working days count anyway)
// ─────────────────────────────────────────────────────────────────────────────
function effectivePresentValue(code) {
  switch (code) {
    case "P":
    case "PL":
    case "PH":
...
router.get("/attendance/report/daily", async (req, res) => {
  ...
  try {
    ...
    const { fullLeaveMap, halfDayLeaveMap } = leaveResult;
    ...
    const empIds = employees.map((e) => e.emp_id);
    const compOffSummaryMap = await fetchCompOffSummaryMap(tenant_id, empIds);
    ...
    const data = employees.map((emp) => {
      ...
      const detail = isSiteEntry ? null : attendanceDetail.get(emp.emp_id);
      const isFullLeave = fullLeaveMap.get(emp.emp_id)?.has(date) ?? false;
      const halfDay = halfDayLeaveMap.get(emp.emp_id)?.get(date) ?? null;
      const isCompOff = compOffUsedMap.get(emp.emp_id)?.has(date) ?? false;
      const isLate = isSiteEntry
        ? ((siteSessionsMap.get(emp.emp_id) ?? [])[0]?.is_late ?? false)
        : detail?.is_late === 1;
      ...
      // Human-readable label for daily table
      const statusLabel =
        {
          P: "Present",
          PL: "Present (Late)",
          PH: "Present + Half Leave",
          PLH: "Present + Half Leave (Late)",
          PU: "Present + Unpaid Half",
          PLU: "Present + Unpaid Half (Late)",
          HD: "Half Day (Paid)",
          HU: "Half Day (Unpaid)",
          L: "Leave",
          A: "Absent",
...
router.get("/attendance/report/matrix", async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      fetchHolidayMapByDate(tenant_id, from, to),
      fetchLeaveMap(tenant_id, from, to),
      fetchAttendanceMap(tenant_id, from, to, attendanceMode),
      fetchCompOffUsedMap(tenant_id, from, to),
    ]);
    ...
    // Phase 2 — summary maps
    const empIds = employees.map((e) => e.emp_id);
    const [compOffSummaryMap, lateSummaryMap, leaveBreakdownMap] =
      await Promise.all([
        fetchCompOffSummaryMap(tenant_id, empIds),
        fetchLateSummaryMap(tenant_id, empIds, from, to, attendanceMode),
        fetchLeaveBreakdownMap(tenant_id, empIds, from, to),
      ]);
    ...
    const data = employees.map((emp) => {
      // Running counters — all decimal to support 0.5
      let presentDays = 0.0; // P/PL/PH/PLH/PU/PLU = 1.0; HD = 1.0; HU = 0.5
      let absentDays = 0; // A only
      let leaveDays = 0.0; // L=1.0; HD/PH/PLH=0.5(paid); HU/PU/PLU=0.5(unpaid)
      let compOffDays = 0;
      let workingDays = 0;
      let effectivePresent = 0.0; // for Att%

      // Deduction tracking (for salary reference)
      let deductionDays = 0.0; // A=1.0; unpaid full L=1.0; unpaid half=0.5
      ...
      const dayStatuses = allDates.map((date) => {
        ...
        // Working day counter (H and W don't count)
        if (!isHoliday && !isWeekend) workingDays++;
        ...
        switch (code) {
          ...
          case "PL":
            presentDays++;
            effectivePresent += 1.0;
            break;

          case "PH":
          case "PLH":
            // Present + paid half leave
            // Salary: full day (present + paid leave) — no deduction
            presentDays++;
            leaveDays += 0.5;
            effectivePresent += 1.0;
            break;
          ...
          case "PU":
          case "PLU":
            // Present + unpaid half leave
            // Salary: full present, but 0.5 deducted for unpaid half
            presentDays++;
            leaveDays += 0.5;
            effectivePresent += 1.0;
            deductionDays += 0.5;
          ...
          case "HD":
            // Paid half-day leave only (absent other half)
            // Salary: paid = full day value → no deduction
            leaveDays += 0.5;
            effectivePresent += 1.0; // paid half = full att% value
            break;

          case "HU":
            // Unpaid half-day leave only (absent other half)
            // Salary: 0.5 day only
            leaveDays += 0.5;
          ...
          case "L":
            // Full-day leave — paid/unpaid tracked in leaveBreakdownMap
            leaveDays += 1.0;
            // effectivePresent += 0 (leave days don't count toward att%)
            // deduction handled by unpaidFullDays in leaveBreakdownMap
            break;
      ...
      // Att% = effectivePresent / workingDays
      const percentage =
        workingDays > 0
          ? parseFloat(((effectivePresent / workingDays) * 100).toFixed(1))
          : 0;
      ...
      // Total deduction = absent days + unpaid full leave + unpaid half leave
      const totalDeductionDays =
        absentDays + lvBreak.unpaidFullDays + lvBreak.unpaidHalfDays * 0.5;
      ...
      return {
        ...
        department: emp.department,
        days: dayStatuses,
        // Summary counts (decimal where applicable)
        present_days: presentDays,
        absent_days: absentDays,
        leave_days: leaveDays,
        comp_off_days: compOffDays,
        ...
        percentage,
        // Leave breakdown
        paid_leave_days: lvBreak.paidLeaveDays,
        unpaid_leave_days: lvBreak.unpaidLeaveDays,
        paid_half_days: lvBreak.paidHalfDays,
        unpaid_half_days: lvBreak.unpaidHalfDays,
        half_day_count: lvBreak.halfDayCount,
        // Salary deduction reference
        deduction_days: totalDeductionDays,
        // Comp-off summary
        comp_off_earned: coSummary.earned,
        comp_off_used: coSummary.used,
        comp_off_expired: coSummary.expired,
