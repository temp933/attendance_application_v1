...
async function fetchEmployees(tenantId, departmentId = null, scopedIds = null) {
  let sql = `
    SELECT
      em.emp_id,
  ...
  if (scopedIds !== null) {
    ...
    sql += ` AND em.emp_id IN (${scopedIds.map(() => "?").join(",")})`;
...
router.get("/attendance/report/daily", async (req, res) => {
  const { tenantId: tenant_id, scopedIds } = await getCallerScope(req);
  ...
  try {
    ...
    const [
      ...
      fetchEmployees(tenant_id, deptId, scopedIds),
