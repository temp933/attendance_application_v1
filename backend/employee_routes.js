...
// ── Role guard factory ────────────────────────────────────────────────────────
// Admin = 1, HR = 2, TL = 3, Employee = 5, AppAdmin = 6, Manager = 8
...
// Query params:
//   status   — Active | Inactive | Relieved  (default: Active)
//   dept_id  — filter by department
//   role_id  — filter by role
//   tl_id    — filter by TL
//   search   — partial match on name / email / emp_id
...
router.get("/", requireAuth, async (req, res) => {
  ...
  if (search) {
    conditions.push(
      `(e.first_name LIKE ? OR e.last_name LIKE ? OR e.email_id LIKE ? OR e.emp_id LIKE ?)`,
    );
    const like = `%${search}%`;
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT
          ...
          CONCAT(t.first_name, ' ', t.last_name) AS tl_name
...
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/employees
// Creates a new employee + login record in a single transaction.
//
// Body (JSON):
...
//             tl_id, username, initial_password
//
// Automatically creates a login_master record.
// If username not supplied, defaults to email prefix.
// If initial_password not supplied, defaults to "Pass@1234" (is_first_login=1).
// ─────────────────────────────────────────────────────────────────────────────
router.post(
  "/",
  requireAuth,
  requireRole(1, 2), // Admin or HR only
  async (req, res) => {
...
// Admin / HR can update anyone; Employee can only update their own record
// (restricted to non-sensitive fields only).
// ─────────────────────────────────────────────────────────────────────────────
router.put("/:emp_id", requireAuth, async (req, res) => {
  const { tenantId, roleId, loginId } = req.user;
  const { emp_id } = req.params;

  // Determine caller's emp_id from login_master
  ...
  for (const field of allFields) {
    ...
    if (!isAdminOrHR && !selfAllowed.has(field)) continue; // skip restricted for self
    fields.push(`${field} = ?`);
  ...
  try {
    ...
    // If role changed, sync login_master role_id
...
// Change employee status: Active | Inactive | Relieved
// Admin / HR only. Relieving also sets date_of_relieving.
// ─────────────────────────────────────────────────────────────────────────────
router.patch(
  "/:emp_id/status",
  requireAuth,
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Disable login on Relieved / Inactive
      if (status !== "Active") {
        await db.query(
          `UPDATE login_master
...
// Soft delete — sets status = 'Inactive'. Hard delete available via ?hard=1.
// Admin only.
// ─────────────────────────────────────────────────────────────────────────────
...
// Stores a face embedding JSON for biometric / face-recognition attendance.
...
// Body: { employees: [ { ...same fields as POST / }, ... ] }
