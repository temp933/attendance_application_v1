  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // Hierarchy helper — returns all emp_ids in the subtree rooted at rootEmpId
  ...
  // GET /api/employees
  // Scoped by hierarchy:
  //   org_admin / app_admin → sees ALL
  //   everyone else         → sees only their subtree (excluding themselves)
  // ─────────────────────────────────────────────────────────────────────────────
  router.get("/", requireAuth, async (req, res) => {
    const { tenantId, loginId } = req.user;
    const roleName = (req.user.role_name || "").toLowerCase().trim();
    const {
      status,
      dept_id,
    ...
    // ── Build scoped ID list for non-admins ───────────────────────────────────
    let scopedIds = null; // null = no restriction
    if (!isFullAccess && callerEmpId) {
      const subtree = await getSubtreeIds(tenantId, callerEmpId);
      // Exclude the caller themselves — they see their reportees, not themselves
      scopedIds = subtree.filter((id) => id !== callerEmpId);
      if (scopedIds.length === 0) {
        // No reportees at all — return empty list immediately
        return res.json({
          success: true,
          total: 0,
          page: safePage,
    ...
    // Inject scope filter
    if (scopedIds !== null) {
      conditions.push(`e.emp_id IN (${scopedIds.map(() => "?").join(",")})`);
    ...
    if (reporting_to_id) {
      conditions.push("e.reporting_to_employee_id = ?");
      params.push(reporting_to_id);
    }
    if (search) {
      conditions.push(
    ...
    try {
      ...
      const [rows] = await db.query(
        `SELECT
            e.emp_id, e.employee_code,
            e.first_name, e.mid_name, e.last_name,
            e.email_id, e.phone_number, e.date_of_birth, e.gender,
            e.designation_id, ds.designation_name,
            ds.department_id, dm.department_name,
            ...
            e.status, e.reporting_to_employee_id, e.has_face_embedding,
        ...
        LEFT JOIN employee_master     t ON t.emp_id           = e.reporting_to_employee_id
        WHERE ${where}
  ...
  router.get("/stats/summary", requireAuth, async (req, res) => {
    ...
    try {
      ...
      const [byDept] = await db.query(
        `SELECT dm.department_name, COUNT(*) AS count
          FROM employee_master e
          JOIN designation_master ds ON ds.designation_id = e.designation_id
          JOIN department_master  dm ON dm.department_id  = ds.department_id
    ...
    try {
      const [rows] = await db.query(
        `SELECT e.emp_id, e.first_name, e.mid_name, e.last_name,
          ...
          LEFT JOIN designation_master ds ON ds.designation_id = e.designation_id
          LEFT JOIN role_master r ON r.role_id = e.role_id
          WHERE e.tenant_id = ? AND ds.department_id = ? AND e.status = ?
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // GET /api/employees/by-reporting-to/:reporting_to_id
  // ─────────────────────────────────────────────────────────────────────────────
  router.get(
    "/by-reporting-to/:reporting_to_id",
    requireAuth,
    async (req, res) => {
      const { tenantId } = req.user;
      const { reporting_to_id } = req.params;
      const { status = "Active" } = req.query;
      ...
      try {
        const [rows] = await db.query(
          ...
          LEFT JOIN department_master  dm ON dm.department_id  = ds.department_id
          LEFT JOIN role_master         r ON r.role_id          = e.role_id
          WHERE e.tenant_id = ? AND e.reporting_to_employee_id = ? AND e.status = ?
          ORDER BY e.first_name`,
          [tenantId, reporting_to_id, status],
        );

        res.json({ success: true, count: rows.length, data: rows });
      } catch (err) {
        console.error("[GET /employees/by-reporting-to]", err);
        res.status(500).json({ success: false, message: "Server error." });
      }
    },
  );
      ...
      const [rows] = await db.query(
        `SELECT DISTINCT e.emp_id AS id,
          ...
          WHERE e.tenant_id = ?
            AND e.status = 'Active'
            AND e.emp_id IN (
              SELECT DISTINCT reporting_to_employee_id
              FROM employee_master
              WHERE reporting_to_employee_id IS NOT NULL
                AND tenant_id = ?
            )
          ORDER BY name`,
        [tenantId, tenantId],
  ...
  router.get("/:emp_id", requireAuth, async (req, res) => {
    ...
    try {
      // ── Hierarchy scope check for non-admins ──────────────────────────────
      const callerRoleName = (req.user.role_name || "").toLowerCase().trim();
      const isFullAccess2 = ["admin", "app_admin", "org_admin"].includes(
        callerRoleName,
      );
      if (!isFullAccess2 && req.user.loginId) {
        const [[lm2]] = await db.query(
          "SELECT emp_id FROM login_master WHERE login_id = ? LIMIT 1",
          [req.user.loginId],
        );
        const callerEmp2 = lm2?.emp_id;
        if (callerEmp2 && String(callerEmp2) !== String(emp_id)) {
          const subtree2 = await getSubtreeIds(tenantId, callerEmp2);
          if (!subtree2.includes(Number(emp_id))) {
            return res
              .status(403)
      ...
      const [rows] = await db.query(
        `SELECT
            e.emp_id, e.tenant_id, e.employee_code,
            e.first_name, e.mid_name, e.last_name,
            e.email_id, e.phone_number, e.date_of_birth, e.gender,
            e.designation_id, ds.designation_name,
            ds.department_id, dm.department_name,
  ...
  router.post("/", requireAuth, requireRole("admin", "hr"), async (req, res) => {
    ...
    const {
      ...
      date_of_birth,
      gender,
      designation_id, // ← was department_id
      role_id,
      date_of_joining,
      date_of_relieving,
      employment_type,
      ...
      esic_number,
      years_experience,
      reporting_to_employee_id, // ← was tl_id
    ...
    // Validate designation belongs to tenant
    const [[desg]] = await db.query(
      "SELECT designation_id FROM designation_master WHERE designation_id = ? AND tenant_id = ? AND status = 'Active' LIMIT 1",
      [designation_id, tenantId],
    );
    if (!desg) {
      return res.status(400).json({
        success: false,
        message: "Invalid designation for this tenant.",
      });
    }
    ...
    try {
      ...
      const [empResult] = await conn.query(
        `INSERT INTO employee_master
          ...
          reporting_to_employee_id, status, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Active', NOW())`,
        [
          tenantId,
          first_name.trim(),
          ...
          nullIfEmpty(reporting_to_employee_id),
        ],
      );
  ...
  router.patch(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const [result] = await db.query(
          `UPDATE employee_master SET status = ? ${extraFields} WHERE emp_id = ? AND tenant_id = ?`,
        ...
        if (status !== "Active") {
          await db.query(
            `UPDATE login_master SET status = 'Inactive', session_token = NULL, device_logged_in = 0 WHERE emp_id = ?`,
            [emp_id],
          );
        } else {
          await db.query(
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      try {
        const [result] = await db.query(
          `UPDATE employee_master SET profile_photo = ?, profile_photo_mime = ? WHERE emp_id = ? AND tenant_id = ?`,
  ...
  router.post(
    ...
    requireAuth,
    requireRole("admin", "hr", "app_admin"),
    async (req, res) => {
      const { tenantId } = req.user;
      const { emp_id } = req.params;
      ...
      try {
        const [result] = await db.query(
          `UPDATE employee_master SET face_embedding = ?, has_face_embedding = 1 WHERE emp_id = ? AND tenant_id = ?`,
