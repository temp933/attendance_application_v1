...
// AUTH MIDDLEWARE
// ─────────────────────────────────────────────────────────────────────────────
// Wraps the shared authMiddleware and normalizes req.user fields used
// throughout this router (loginId, tenantId, roleId, empId, companyId).
// Falls back to x-tenant-id header if tenant_id isn't present on req.user
// (covers app_admin / cross-tenant style requests).
function requireAuth(req, res, next) {
  authMiddleware(req, res, () => {
    if (!req.user) {
      return res.status(401).json({
    ...
    // NOTE: role_id here is tenant-scoped & auto-incremented — do NOT use
    // this for cross-tenant authorization checks, only role_name is safe.
    req.user.roleId = parseInt(req.user.role_id, 10);
    req.user.empId = req.user.emp_id;
    req.user.companyId = req.user.tenant_id ?? req.headers["x-tenant-id"];
...
// Role-name-based gate (NOT role_id — role_id is per-tenant and has no
// global meaning, see "Role-name-based authorization" refactor).
...
// DATE VALIDATION
// ─────────────────────────────────────────────────────────────────────────────
// Throws if value is a non-empty string that isn't strictly yyyy-MM-dd.
// Null/undefined/"" are treated as "not provided" and pass silently —
// callers decide whether the field is actually required.
function assertDateString(value, fieldName) {
  if (value === null || value === undefined || value === "") {
    return;
  }
...
// AUTO REJECT HELPER
// ─────────────────────────────────────────────────────────────────────────────
// Used when approval hits a hard validation failure (e.g. duplicate email)
// instead of just bombing out — auto-marks the pending request REJECTED
// with a reason, commits, and returns a {status, body} pair for the caller
// to res.status(...).json(...) directly.
async function autoReject(conn, request_id, reason, errorMsg) {
  await conn.query(
    `
          UPDATE employee_pending_request
...
// APPLY EDUCATION CHANGES
// ─────────────────────────────────────────────────────────────────────────────
// Applies staged education edits from education_pending_request onto the
// real education_details table for a given request_id.
//
// Each staged row carries action_type (ADD / UPDATE / DELETE),
// original_edu_id (the real edu_id being touched, for UPDATE/DELETE),
// and is_changed (0/1) — this avoids destructive delete-and-reinsert and
// lets us skip UPDATE rows that weren't actually modified.
//
// Returns true if any pending education rows existed for this request,
// false otherwise (caller can use this to decide whether to fall back to
// master data when building diffs).
async function applyEducationChanges(conn, request_id, empId, tenantId) {
  const [eduRows] = await conn.query(
    `
          SELECT *
  ...
  for (const edu of eduRows) {
    // Skip UPDATE rows that were staged but never actually changed by the
    // user — prevents no-op writes / churning updated_at unnecessarily.
    if (edu.is_changed === 0 && edu.action_type === "UPDATE") {
      continue;
    }
    ...
    switch (edu.action_type) {
      case "ADD":
        // Brand new education record — insert fresh.
        await conn.query(
          `
                INSERT INTO education_details
                (
      ...
      case "UPDATE":
        // Update existing record identified by original_edu_id.
        await conn.query(
          `
                UPDATE education_details
                SET
      ...
      case "DELETE":
        // Remove the record the user marked for deletion.
        await conn.query(
          `
                DELETE FROM education_details
                WHERE edu_id = ?
...
// GET PENDING REQUESTS
// ─────────────────────────────────────────────────────────────────────────────
// Returns all PENDING requests (NEW employee or UPDATE) for the tenant,
// with field-level COALESCE(pending_value, current_master_value) so the
// UI always has a value to show even if a field wasn't touched in the edit.
//
// For each row we additionally attach:
//   - education_list: staged education rows (NEW) or pending/master rows (UPDATE)
//   - current_data: full current employee_master snapshot (UPDATE only),
//     used by the frontend to render an old-vs-new diff/highlight view.
router.get(
  "/pending-requests",
  requireAuth,
  requireRole("admin", "hr"),
  async (req, res) => {
    ...
    try {
      const [rows] = await db.query(
        `
          ...
          FROM employee_pending_request p
            ...
            -- Existing employee record (NULL for NEW request_type)
            LEFT JOIN employee_master e
              ON e.emp_id = p.emp_id
            AND e.tenant_id = p.tenant_id

            -- Reporting manager — prefer pending value, fall back to current master
            LEFT JOIN employee_master tl
              ON tl.emp_id = COALESCE(p.reporting_to_employee_id, e.reporting_to_employee_id)

            -- Designation/role/department resolved using pending value first,
            -- falling back to the employee's current value
            LEFT JOIN designation_master d
              ON d.designation_id =
                COALESCE(
                  p.designation_id,
      ...
      // For each pending request, hydrate education_list and (for
      // UPDATE-type requests) current_data so the frontend can render
      // an old-vs-new diff view.
      for (const row of rows) {
        // EDUCATION (NEW employee — just show what was staged)
        if (row.request_type === "NEW") {
          const [newEduRows] = await db.query(
            `SELECT education_level, stream, score, year_of_passout, university, college_name,
                      action_type, is_changed, original_edu_id
        ...
        // CURRENT EMPLOYEE DATA (UPDATE requests only — used for diffing)
        if (row.request_type === "UPDATE" && row.emp_id) {
          const [[current]] = await db.query(
            `
                SELECT
          ...
          if (current) {
            // Attach the employee's current (master) education list to
            // current_data so the frontend has a true "before" snapshot.
            const [masterEdu] = await db.query(
              `SELECT education_level, stream, score, year_of_passout, university, college_name
                FROM education_details
                WHERE emp_id = ? AND tenant_id = ?
          ...
          const hasAnyPendingEdu = pendingEduRows.length > 0;
          if (hasAnyPendingEdu) {
            // User staged education changes — show those (drives the diff UI)
            row.education_list = pendingEduRows;
          } else {
            // No edu rows submitted at all — show master data as-is (no diff)
            const [masterEduFallback] = await db.query(
...
// APPROVE REQUEST
// ─────────────────────────────────────────────────────────────────────────────
// Approves a pending NEW or UPDATE request inside a single transaction.
//   - NEW: creates employee_master + login_master rows, applies staged
//     education rows, then marks the pending request APPROVED.
//   - UPDATE: patches employee_master (only fields that are non-null in
//     the pending row), syncs login_master.role_id if role changed,
//     applies staged education changes, then marks APPROVED.
// Any hard failure (e.g. duplicate email on NEW) auto-rejects via
// autoReject() instead of leaving the request stuck in PENDING.
router.post(
  "/approve-request",
  requireAuth,
  requireRole("admin", "hr"),
  async (req, res) => {
    ...
    try {
      ...
      // Lock in the pending row we're about to act on — re-check
      // admin_approve = 'PENDING' here to avoid double-approving.
      const [[pending]] = await conn.query(
        `
              SELECT *
              FROM employee_pending_request
      ...
      // Guard against malformed dates slipping through to MySQL.
      assertDateString(pending.date_of_birth, "date_of_birth");
      ...
      // ───────────────────────────────────────────────────────────────────────
      if (pending.request_type === "NEW") {
        // Guard against duplicate email across the tenant before creating
        // the employee record.
        const [[dupEmail]] = await conn.query(
          `
                SELECT emp_id
        ...
        // Generate employee_code: EMP0001, EMP0002, … (tenant-scoped,
        // based on current count — not gap-safe, but fine for sequential codes)
        const [[{ empCount }]] = await conn.query(
          `SELECT COUNT(*) AS empCount FROM employee_master WHERE tenant_id = ?`,
          [tenantId],
        );
      ...
      `,
        ...
        // Create the login account tied to this new employee.
        // is_first_login = 1 forces a password reset / onboarding flow.
        await conn.query(
          `
                INSERT INTO login_master
                (
        ...
        // Push staged education rows into education_details for the new employee.
        await applyEducationChanges(conn, request_id, newEmpId, tenantId);
      ...
      // Build a dynamic SET clause from only the fields that were actually
      // submitted in the pending request (non-null/undefined) — avoids
      ...
      // Only overwrite the profile photo if a new one was actually staged.
      if (pending.profile_photo) {
        updatable.push(["profile_photo", pending.profile_photo]);
      ...
      // Keep login_master.role_id and contact_number in sync with
      // employee_master — login_master is the source of truth used at auth time.
      ...
      // Apply any staged education ADD/UPDATE/DELETE rows.
      await applyEducationChanges(conn, request_id, empId, tenantId);
      ...
      // Any failure anywhere in NEW/UPDATE handling rolls back the whole
      // transaction — request stays PENDING for retry.
      await conn.rollback();
...
// REJECT REQUEST
// ─────────────────────────────────────────────────────────────────────────────
// Marks a PENDING request as REJECTED with a required reason. Does not
// touch employee_master/login_master at all — purely a status + reason update.
router.post(
  "/reject-request",
  requireAuth,
  requireRole("admin", "hr"),
  async (req, res) => {
    ...
    try {
      // WHERE clause re-checks admin_approve = 'PENDING' so a request
      // already approved/rejected can't be flipped again.
      const [result] = await db.query(
        `
              UPDATE employee_pending_request
              SET
