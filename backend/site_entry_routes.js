...
// // ─────────────────────────────────────────────────────────────────────────────
// // Auth helpers  (mirrors gps_face_routes.js pattern)
// // ─────────────────────────────────────────────────────────────────────────────
...
// /** Parse a polygon_json string and return its centroid {lat, lng} */
...
// /**
//  * Given lat/lng, return all tenant sites that the point is within 50 m of
//  * (uses polygon containment first; falls back to centroid distance).
//  */
...
// // ─────────────────────────────────────────────────────────────────────────────
// // GET /api/site-entry/nearby-sites?lat=&lng=&radius=50
// // ─────────────────────────────────────────────────────────────────────────────
...
// // Returns whether the employee already face-verified for site_entry today
// // ─────────────────────────────────────────────────────────────────────────────
...
// // All sessions for this employee today (multiple sites possible)
// // ─────────────────────────────────────────────────────────────────────────────
...
// // ─────────────────────────────────────────────────────────────────────────────
// // POST /api/site-entry/checkin
// // Body: { site_id, latitude, longitude }
// // Creates a new session for the given site.
// // If an active session exists for a DIFFERENT site, that one is auto-checked-out.
// // ─────────────────────────────────────────────────────────────────────────────
...
//     if (sameActive) {
//       // Already active on the same site — just resume if paused
...
//     // Check late status using attendance policy
//     const [[policy]] = await db.query(
//       `SELECT office_in_time, late_after_minutes FROM attendance_policy WHERE tenant_id = ? LIMIT 1`,
//       [tenantId],
//     );

//     let isLate = 0,
//       lateMinutes = 0;
//     // Only mark late for the very first check-in of the day
...
// // ─────────────────────────────────────────────────────────────────────────────
// // PATCH /api/site-entry/update-location
// // Body: { latitude, longitude }
// //
// // Called every 2 minutes by the background service.
// // Handles:
// //   1. Employee still on same site     → update last_known coords, clear pause
// //   2. Employee left site < 10 min ago → still paused, just update coords
// //   3. Employee left site > 10 min ago → auto-checkout
// //   4. Employee now at a NEW site      → auto-checkout old, auto check-in new
// //   5. Employee not at any site        → start/extend pause timer
// // ─────────────────────────────────────────────────────────────────────────────
...
//     if (!active) {
//       // Nothing to do — check-in must be manual (with face verify)
//       return res.json({
//         success: true,
...
//       if (active.paused_at) {
//         // Employee returned — resume: accumulate pause time, clear paused_at
...
//       // Create new session for the new site (no face verify needed — already done today)
...
// // ─────────────────────────────────────────────────────────────────────────────
// // GET /api/site-entry/history?limit=20&offset=0
// // ─────────────────────────────────────────────────────────────────────────────
...
// // ─────────────────────────────────────────────────────────────────────────────
// // GET /api/site-entry/admin/all?date=&site_id=&status=&search=&limit=&offset=
// // ─────────────────────────────────────────────────────────────────────────────
...
//
// Policy gates:
//   multiple_in_out_allowed = 0  →  block if any session (active OR completed)
//                                   already exists today for this employee
//   multiple_in_out_allowed = 1  →  original behaviour (allow re-entry)
// ─────────────────────────────────────────────────────────────────────────────
router.post("/checkin", requireAuth, async (req, res) => {
  const { tenantId, empId } = req.user;
  const { site_id, latitude, longitude } = req.body;
...
//
// Policy gates:
//   auto_checkout_enabled = 0  →  never auto-checkout on absence; keep pausing
//                                 indefinitely (employee must manually check out)
//   auto_checkout_enabled = 1  →  auto-checkout after >10 min absence (original)
//
// Full flow:
//   1. Still on same site            → update coords, clear pause
//   2. Left < 10 min ago             → still_paused, update coords
//   3. Left > 10 min, auto=1         → auto_checked_out
//   4. Left any duration, auto=0     → extend_pause (keep session alive)
//   5. Moved to a different site     → auto-checkout old, auto check-in new
//   6. No active session             → no_active_session
// ─────────────────────────────────────────────────────────────────────────────
router.patch("/update-location", requireAuth, async (req, res) => {
  const { tenantId, empId } = req.user;
  const { latitude, longitude } = req.body;
  ...
  try {
    ...
    if (!autoCheckoutEnabled) {
      // Policy: auto_checkout_enabled = 0
      // Keep the session alive — just update coords, do NOT close
      await db.query(
        `UPDATE employee_attendance
         SET last_known_latitude      = ?,
             last_known_longitude     = ?,
